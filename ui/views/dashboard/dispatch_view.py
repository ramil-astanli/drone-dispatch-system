...
async def build_dispatch_view(page: ft.Page) -> ft.Control:
    ...
    pkg_dd = ft.Dropdown(
        ...
        options=[
            ft.dropdown.Option(
                key=str(p["id"]),
                text=f"#{p['id']}  —  {p['description']}  ({p['weight']} kg, {p['priority']})",
            )
            for p in packages
        ],
        border_radius=8,
    ...
    async def on_dispatch(_=None) -> None:
        ...
        has_err = False
        if not pkg_dd.value:
            pkg_dd.error_text = "Select a package"; has_err = True
        if not origin_f.value or not origin_f.value.strip():
            origin_f.error_text = "Required"; has_err = True
        if not dest_f.value or not dest_f.value.strip():
            dest_f.error_text = "Required"; has_err = True
                ...
                ft.Card(
                    content=ft.Container(
                        content=ft.Row(
                            [
                                ft.Container(
                                    content=ft.Icon(ft.Icons.CHECK_CIRCLE_OUTLINED,
                                                    color=ft.Colors.GREEN_400, size=28),
                                    padding=ft.Padding.only(right=4),
                                ),
                                ft.Column(
                                    [
                                        ft.Text(f"Route #{route['id']} created",
    ...
    history.controls = [
        ft.Container(
            content=ft.Text("Dispatched routes will appear here.",
                            color=ft.Colors.GREY_600, size=13, italic=True),
    ...
    return ft.Column(
        [
            section_header("Dispatch Console",
                           "Assign the best available drone to a package"),
                                    ...
                                    ft.Container(
                                        content=ft.Row(
                                            [
                                                ...
                                                ft.Text(
                                                    "The system will select the IDLE drone "
                                                    "with the highest battery (> 25 %).",
